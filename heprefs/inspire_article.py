...
class InspireArticle(object):
    ...
    DATA_KEY = (
        "primary_report_number,recid,system_control_number,"
    ...
    def get_info(cls, query):
        query_url = "{}?p={}&of=recjson&ot={}&rg=3".format(
            cls.API, quote_plus(query), cls.DATA_KEY
        )
        try:
        ...
        try:
            ...
            raise Exception(
                "parse failed; query {} to inspireHEP gives no result?: ".format(query)
                + e.__str__()
            )
    ...
    def pdf_url(self):
        ...
        scoap3_url = [
            i["url"]
            for i in self.info.get("files", [])
            if i["full_name"] == "scoap3-fulltext.pdf"
        ]
        if scoap3_url:
            return scoap3_url[0]
        ...
        pdf_files = [
            i for i in self.info.get("files", []) if i["superformat"] == ".pdf"
        ]
        if pdf_files:
            if len(pdf_files) > 1:
    ...
    def download_parameters(self):
        ...
        names = (
            invenio.shorten_authors_text(self.info)
            .replace(", ", "-")
            .replace("et al.", "etal")
        )
