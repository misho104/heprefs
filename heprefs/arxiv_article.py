...
class ArxivArticle(object):
    ...
    def get_info(cls, arxiv_id):
        ...
        if results.get("status") != 200:
            raise Exception(
                "Failed to fetch arXiv article information: HTTP Error "
                + str(results.get("status", "no status"))
            )

        if (
            len(results["entries"]) == 0 or "id" not in results["entries"][0]
        ):  # because arXiv api returns one blank entry even if nothing found
